...
function Generator() {
  ...
  return yeoman.generators.Base.extend({
    constructor: function () {
      ...
      if (!config) {
        this.log('Generator ' + self.generator + ' not found.');
      } else {
        this.config = config;
    ...
    prompting: function () {
      if (this._generator && this._generator.prompting) {
        this._generator.prompting.apply(this, arguments);
      }
    },

    writing: function () {
      if (this._generator && this._generator.writing) {
        this._generator.writing.apply(this, arguments);
      }
    },

    install: function () {
      if (this._generator && this._generator.install) {
...
function promptRunner(generatorContext, prompts, callback) {
  ...
  generatorContext.prompt(_prompts, function (props) {
    ...
    var transform = generatorContext.config.transform;

    if (transform) {
      props = transform(props) || props;
    }
...
function getSrcAndDst(item, generatorContext, repository) {
  ...
  return {
    when: when,
    src: ejs.render(src, generatorContext.props),
